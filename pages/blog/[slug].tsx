...
export default function SingleArticlePage(props: InferGetStaticPropsType<typeof getStaticProps>) {
  ...
  const { slug, mdxSource, post } = props;

  if (!post) {
    return null;
  }
  const { title, description, date, tags, imageUrl } = post.meta as NonNullableChildrenDeep<typeof post.meta>;
  ...
  return (
    <>
      ...
      <CustomContainer id="content" ref={contentRef}>
        ...
        <MDXRichText mdxSource={mdxSource} />
      </CustomContainer>
    </>
  );
}
